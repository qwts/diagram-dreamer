...
const ResizableHandle = ({
  ...
  <Separator
    className={cn(
      // `after:left-1/2` with `after:-translate-x-1/2` is the same symmetric
      // centering pair documented in dialog.tsx: it widens the 1px separator's
      // hit area around its own centre, so it is direction-agnostic and a
      // `start-1/2` rewrite would break RTL rather than fix it. The genuinely
      // directional sibling below is logical (`after:start-0`).
