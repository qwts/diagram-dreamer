...
4. **Zero hardcoded user-facing strings.** Every string through i18next, resources bundled synchronously at init. Logical CSS properties only (`ms-/me-/ps-/pe-`, `start/end`) — with one exception: styling keyed to a `data-side` attribute that a library has already _resolved_ (Radix popovers) must stay physical, because the flip has happened once already and a logical value would apply it twice. Exceptions are documented at the code, not assumed.
